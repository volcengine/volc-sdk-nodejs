...
```typescript
  ...
  // 设置aksk
  tlsOpenapiService.setSecretKey('your SecretKey');
  tlsOpenapiService.setAccessKeyId('your AccessKeyId');
...
### Project
```typescript
  interface IDescribeProjectsReq {
    /** 分页页码 */
    PageNumber?: number;
    /** 页面大小 */
    PageSize?: number;
  ...
  // 获取项目列表
  tlsOpenapiService.DescribeProjects(IDescribeProjectsReq);
  ...
  // 创建项目
  tlsOpenapiService.CreateProject(IProjectCreateReq);

  interface IProjectModifyReq {
    /** 日志项目描述信息: 不支持<>、'、\、\\; 长度为0～64个字符。 */
    Description?: string;
    /** Project的uuid。 */
    ProjectId: string;
  ...
  // 修改项目
  tlsOpenapiService.ModifyProject(IProjectModifyReq);
...
```typescript
  ...
  // 获取日志主题列表
  tlsOpenapiService.DescribeTopics(IDescribeTopicsReq);
  ...
  // 创建日志主题
  tlsOpenapiService.CreateTopic(ITopicCreateReq);

  interface ITopicModifyReq {
    /** 日志主题描述信息：不支持<>、'、\、\\；长度为0-64个字符。 */
    Description?: string;
    /** 日志主题ID。 */
    TopicId: string;
  ...
  // 修改日志主题
  tlsOpenapiService.ModifyTopic(ITopicModifyReq);
...
```typescript
  ...
  tlsOpenapiService.CreateIndex(IIndexCreateReq);

  // 修改主题索引
  type IIndexModifyReq = IIndexCreateReq;
  tlsOpenapiService.ModifyIndex(IIndexModifyReq);
...
```typescript
  ...
  tlsOpenapiService.DescribeShards(IDescribeShardsReq);
```
...
```typescript
interface ITraceInsDescribeResp {
  CreateTime: string;
  ...
  TraceInstanceStatus: Enums.TraceInstanceStatus;
  TraceTopicId: string;
  TraceTopicName: string;
}

interface IDescribeTraceInstancesReq {
  /** 分页，默认从1开始。 */
  PageNumber?: number;
  /** 分页大小限制，默认为20，最大为100。 */
  PageSize?: number;
...
tlsOpenapiService.DescribeTraceInstances(IDescribeTraceInstancesReq);

interface ITraceInsCreateReq {
...
tlsOpenapiService.CreateTraceInstance(ITraceInsCreateReq);

interface IDescribeTraceInstanceReq {
  /** Trace实例id */
  TraceInstanceId: string;
}

tlsOpenapiService.DescribeTraceInstance(IDescribeTraceInstanceReq);

interface ITraceInsModifyReq {
  /** 日志主题描述信息：不支持<>、'、、；长度为0-64个字符。 */
  Description?: string;
  /** Trace实例ID。 */
  TraceInstanceId: string;
...
tlsOpenapiService.ModifyTraceInstance(ITraceInsModifyReq);

interface ITraceInsDeleteReq {
  /** Trace实例ID。 */
  TraceInstanceId: string;
}

tlsOpenapiService.DeleteTraceInstance(ITraceInsDeleteReq);
```
